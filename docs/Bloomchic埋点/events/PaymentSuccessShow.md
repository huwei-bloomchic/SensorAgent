...
- **事件显示名**: ThankYou页,订单详情页展示
- **事件英文名**: `PaymentSuccessShow`
...
| --- | --- | --- | --- | --- | --- | --- | --- |
| `product_quantity` | 商品数量 | NUMBER | 商品总数 | JavaScript | 订单ThankYou页,订单详情页页面展示时上报 |  | Checkout 升级<br/>这个用回原来的，不做处理<br/>数据结构用回原来的 只是无法统计“支付类型”跟“支付方式名”<br/>增加： page_type ，用于区分所在页面类型 |
...
| `total` | 总计金额 | NUMBER |  |  |  |  |  |
| `page_type` | 页面类型 | STRING | thank_you | checkout_detail | JavaScript |  |  |  |
